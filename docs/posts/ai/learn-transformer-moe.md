...
1. **打地基**：神经网络 → 神经元 → 偏置 → 前向传播 → 激活函数 → 损失函数 → 交叉熵 → 梯度下降 → 链式法则 → 反向传播 → 优化器 → 学习率 → Softmax
