...
| 术语 | 大白话 |
|------|--------|
| **神经元（neuron）/ 节点** | 委员会里的一个「委员」：接收一些数，做一次简单计算，吐出一个数。它本身极简单，厉害的是成千上万个连起来。 |
| **权重（weight）** | 委员对某个线索的「话语权」——AI 里是**相乘的系数**（不是日常说的权力/分量）。权重大，这个线索对结论的影响就大。 |
| **偏置（bias）** | 委员本身「标准的高低」——AI 里是**相加的底数**。偏置高，委员更容易「开火」下结论。 |
| **激活函数（activation）** | 委员「要不要把印象往下传」的开关：输入够强才放行，太弱就按住。它还让网络能学**非线性**的复杂规律——不然再多委员叠起来，也只能画出一条直线。 |
| **层（layer）** | 一道工序：同一层的委员各自独立算，再把结果交给下一道。分输入层、隐藏层（中间若干道）、输出层。 |
| **训练 / 学习（假熟词）** | 这些词在 AI 语境和日常不一样。「训练 / 学习」不是人那样读书思考，而是指**用数据反复调网络里的权重和偏置，让输出越来越准**——文末代码里 `loss.backward()` + `opt.step()` 那两行，就是这件事的真实模样。 |
