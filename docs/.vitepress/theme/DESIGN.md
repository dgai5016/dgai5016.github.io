...
| 色 | 值 | 场景 |
| --- | --- | --- |
| 读完绿 | `#16a34a` | 书单详情浮层的「读完」文字徽章（卡片上的对勾角标用主题紫 accent） |
...
| 状态 | 封面 | 文字 | 角标 |
| --- | --- | --- | --- |
| 未读 | `filter: grayscale(1)` + `opacity: 0.72` | 书名/作者转 muted 灰 | 无 |
| 读完（`isRead: true`） | 原色 | 正常深色 | 封面右上角 20px 主题紫圆白勾（`var(--c-accent)`，SVG 自绘 stroke 风但 stroke-width 3.5，带白描边环防彩色封面撞色） |
| hover 未读 | `filter: none` 恢复彩色（0.2s） | — | — |

封面两枚角标对称：**左上角序号**（主题内 1 起，18px 白玻璃胶囊数字 `--c-text-secondary`，两位数自动撑宽，白描边环与对勾同款）、**右上角已读对勾**（仅读完的书）。序号比对勾小一号——序号是辅助信息，视觉权重让给状态。

排序（sortBooks）：**已读置顶优先**，其余保持 yaml 书写顺序（难度等级字段 level 已于 2026-09 移除）；置顶后序号 1、2 天然落在读完的书上，与对勾形成「彩色 + 双角标」的最强识别组合。
