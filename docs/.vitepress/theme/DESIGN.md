...
| 色 | 值 | 场景 |
| --- | --- | --- |
| 读完绿 | `#16a34a` | 书单预览卡的「读完」文字徽章（卡片上的已读标识用紫色序号徽章，不占此色） |
| 京东红 | `#e1251b` | 京东胶囊（hover `#c81623`） |
| 豆瓣绿 | `#00b51d` | 豆瓣胶囊（hover `#009c19`） |
| 微信读书绿 | `#07c160` | 微信读书胶囊（hover `#06ad56`）；比豆瓣绿亮，相邻可区分 |

**可视化工作台色板（唯一的多色豁免）**：`MM_COLORS = ['#6c63ff', '#0ea5e9', '#10b981', '#f59e0b', '#ec4899', '#8b5cf6']`（首色站点主题紫 + 5 个柔和区分色），仅限全屏可视化场景——书单思维导图（BookMindmap 按深度循环取色）与文章地图星图（PostMap 每个一级合集一色、子树继承）。常规 UI 元素仍守单 accent，不得引用此板。
...
| 大浮层 | `1rem`(16px) | desktop-sidebar、palette 面板 |
...
- **两种形态**：右滑面板（PostOverlay `min(100%,72rem)`、BilingualOverlay `min(94%,92rem)`，0.32s custom bezier）/ 居中模态（Lightbox、CommandPalette `36rem`、代码展开）
...
- **全屏导图工作台**（`BookMindmap.vue`）：同范式变体——顶栏（书名 + 工具胶囊 + ×）+ 大画布（Markmap 渲染）；markmap 库动态 import（避 SSR + 代码分割）；**markmap-view 0.18 的 `color` option 只接受函数**（传数组会炸 `n is not a function` 导致整图空白），工具条切换展开深度用 `renderData()` 而非 `setData()`（后者会按 initialExpandLevel 重算折叠覆盖手动状态）
...
分层逻辑：内容(auto) < 返回钮(20) < 抽屉遮罩(30) < 抽屉(40) < hamburger(50) < 右滑面板(100系) < 全屏模态(200系)。
...
**单层浏览交互（详情卡，非模态）**：**点击**书籍卡片 → 卡片旁弹出详情卡 `.book-detail-card`（340px 毛玻璃卡）。**无遮罩、页面照常可点可滚**：点别的书直接切换、点空白/Esc 关闭、**页面滚动超 8px 即收起**（卡是 fixed 不跟书卡走，滚动即关闭避免脱节——非模态 popover 标准行为）；点卡内元素（胶囊、章节文章链接）不关，文章读完回来卡还在；Esc 分层（覆盖层开着时让给它）。布局两段：**头部**（封面+书名+元信息+胶囊行含思维导图按钮）固定不滚动，**滚动区**（章节文章列表）限高「6 整行 + 半行 peek」暗示可滚，列表滚到头继续滚自然过渡为滚页面→关闭。定位两步：顶对齐书卡 → 渲染后按实际卡高精调出界部分。右侧放不下自动翻左侧。（历史：hover 卡 → 遮罩轻模态 → 非模态，2026-09 三轮迭代定型）

**思维导图**：yaml 书条目带 `mindmap: <slug>` 字段 → 详情卡显示「🧠 思维导图」主题紫胶囊（站内功能用主题色，区别于三平台品牌色）；点击开全屏 Markmap 工作台（详情卡保持开着）。导图 md 存 `public/books/<slug>/mindmap.md`（运行时 fetch 原文），由 dg-book-mindmap / dg-book-study 技能从书籍 markdown 电子版生成。

排序（sortBooks）：**已读置顶优先，已读之间按 finishedDate（读完年月，来自读书笔记的 created）先读完的在前**，其余保持 yaml 书写顺序（难度等级字段 level 已于 2026-09 移除）；置顶后紫色序号从读书历程最早的一本数起，「彩色封面 + 紫色序号」即「已读完」标识，预览卡元信息尾部展示「读完于 YYYY-MM」。

数据字段：yaml `isRead: true`（2026-09 由 `status: 读完` 改名），完整文字语义见预览卡。

## 14. 全屏浮层：文章地图（径向星图）

`PostMap.vue` 全屏浮层（fixed inset-0 + glass-card 底），**无按钮入口**，全局快捷键 **Ctrl+L** 开关（Layout 挂 keydown，toggle 语义；排除 Cmd/Alt/Shift 组合防误触，单按 l 不触发）。选 Ctrl 而非 Cmd：Cmd+L 是浏览器保留键（聚焦地址栏），页面拦不住；macOS 浏览器对 Ctrl+L 无默认绑定、可可靠拦截。浮层内左下 hint 写明快捷键（唯一可发现性提示）。归档页保留但 nav 入口隐藏（同「关于」先例）。

- **z-index 分层**：地图层 90——盖过常驻按钮（hamburger/地图钮 50），但**低于右滑阅读浮层（PostOverlay 100）**：在地图里点文章，浮层滑出压在地图上可读，关掉浮层回到地图。Esc 分层同理（浮层开着时 Esc 只关浮层）
- **数据层 `map.data.ts`**：build 期 fs + regex 读 frontmatter 的 tags 分组（兼容行式 `tags: [AI]` 与块式两写法）+ hidden 过滤，自动构建「根 → 标签分类 → 文章」两层树，**零手工登记**。粒度约定（2026-09 与 dg 对齐）：只细化到首页显示的文章，合集内部条目（教程子文章/译文页/开源库条目）不进图
- **节点尺寸按深度**：根 34（accent 实心）/ 分类 16（白底彩边 3px）/ 文章叶子 8；半径档 [0, 250, 460]；色取 MM_COLORS（见第 2 节豁免），每分类一色、子树继承
- **动态 viewBox**：高度基准 1150、宽度按视口宽高比拉宽（`fitCanvas()` 打开/挂载时计算）——正方形 viewBox 在 16:9 屏会被高度 letterbox 卡住，动态适配让短边撑满全屏
- **交互**：点击分类节点折叠↔展开（初始全展开）；文章叶子是真实 `<a>`——**站内链接必须带 `target="_self"`**（VitePress capture 拦截不带 target 的站内 a 做 SPA 路由，点击会跳走；带上即跳过接管，配合 `preventDefault` + `openOverlay` 右滑浮层，与 PostLink 同款），修饰键放行原生新标签；**触控板/滚轮手势按 macOS 地图惯例分流**——双指滑动或滚轮（普通 wheel）= 平移（delta 取反，内容跟随手指）、双指捏合（ctrl+wheel）= 缩放（围绕指针点 0.35~3x，`Math.exp(-deltaY*0.008)` 细粒度步长，`getScreenCTM` 逆变换）；空白拖拽平移用 pointer 事件统一鼠标/触摸，位移 >4px 判拖拽并吞掉随后的 click；悬停高亮祖先链路（链路外元素降到 0.12/0.3 透明度）
- **工具条**：右上玻璃胶囊组（展开全部 / 收起 / 适配屏宽 ｜ 分隔线 ｜ 关闭×，hover 图标旋转 90° 范式），对齐 BookMindmap 工作台范式；左下操作提示小玻璃条（<640px 隐藏）；打开时锁 body 滚动
