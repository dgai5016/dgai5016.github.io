...
**单层浏览交互（hover card 即详情层）**：hover 卡片 **1s**（或点击卡片立即）→ 卡片旁弹出预览卡 `.book-hover-card`（340px 毛玻璃小卡：封面 + 书名 + 读完徽章 + 元信息 + 京东/豆瓣/微信读书胶囊 + 笔记链接）。预览卡**可停留**：鼠标移上去保持显示（离开预览卡才消失），卡片与预览之间的 12px 间隙靠「离开卡片延迟 250ms 隐藏」过渡；右侧放不下自动翻左侧，底部出视口上提。Esc 可收起。原则：**一切浏览与跳转都在轻层完成，不做全屏模态**（全屏详情浮层已于 2026-09 移除）。
