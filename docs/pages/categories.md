...
<div class="flex flex-wrap gap-2 mb-8">
  <button
    ...
    :class="[
      ...
      !activeCategory ? 'bg-accent text-white' : 'bg-bg-dark-surface text-text-dark-muted hover:text-accent'
    ]"
  >
    全部
  </button>
  <button
    ...
    :class="[
      'px-4 py-2 rounded-lg text-sm transition-all',
      activeCategory === cat.slug ? 'bg-accent text-white' : 'bg-bg-dark-surface text-text-dark-muted hover:text-accent'
    ]"
  >
    {{ cat.name }}
