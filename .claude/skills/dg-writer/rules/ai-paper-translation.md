...
8. **特殊 token 抓取残留**：`<BOS>`/`<EOS>` 等尖括号 token 在中间稿里可能变成空的 `$$ and $$`，对照 ar5iv 原文恢复为行内代码。**同类扩大版**：系统类论文正文里的裸占位符 token（`<path>`、`<session_id>`、`<system-reminder>`、`<project-id>` 等）必须全部包进反引号——裸写会被 Vue 模板解析成未闭合标签、整页编译报 "Element is missing end tag"（2026-09 OpenDev 论文踩过，一批 10+ 处）；注意排除合法 HTML 标签（`<u>` 斜体、`<b>`/`<em>` 等按白名单跳过）。
