...
| `fetch-originals.py` | 抓官方 markdown/MDX → 清洗 Docusaurus 语法（剥 frontmatter/import、`<Tabs>/<TabItem>` 转加粗小标题、`<APITable>` 拆壳、相对链接绝对化）→ 巨页按 H2 切片 → 落盘 `en/`。支持过滤参数增量抓取 |
...
3. `python3 scripts/ragflow-docs/gen-manifest.py` 重新生成前端清单
4. 文章加条目（注意后续条目编号顺延 +1）
5. 翻译 → 配对（同场景 A 的 3~4 步）
